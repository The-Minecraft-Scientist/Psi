...
public class GuiProgrammer extends Screen {
	...
	static {
		...
		LAYER = RenderType.create(LibMisc.PREFIX_MOD + LibBlockNames.PROGRAMMER, DefaultVertexFormat.POSITION_COLOR_TEX_LIGHTMAP, VertexFormat.Mode.QUADS,128, true, true, glState);
	}
	...
	protected void init() {
		...
		statusWidget = addWidget(new StatusWidget(left - 48, top + 5, 48, 30, "", this));
		spellCostsWidget = addWidget(new SpellCostsWidget(left + xSize + 3, top + (takingScreenshot ? 40 : 20), 100, 126, "", this));
		panelWidget = addWidget(new PiecePanelWidget(0, 0, 100, 125, "", this));
		helpButton = addWidget(new GuiButtonHelp(left + xSize + 2, top + ySize - (spectator ? 32 : 48), this));
		configWidget = addWidget(new SideConfigWidget(left - 81, top + 55, 81, 115, this));

		spellNameField = addWidget(new CallbackTextFieldWidget(getMinecraft().font, left + xSize - 130, top + ySize - 14, 120, 10, button -> {
		...
		commentField = addWidget(new CallbackTextFieldWidget(getMinecraft().font, left, top + ySize / 2 - 10, xSize, 20, button -> {
		...
		panelWidget.searchField = addWidget(new CallbackTextFieldWidget(getMinecraft().font, 0, 0, 70, 10, button -> {
			panelWidget.page = 0;
		...
		*/
		...
		addWidget(new GuiButtonIO(left + xSize + 2, top + ySize - (spectator ? 16 : 32), true, this, button -> {
			if (hasShiftDown()) {
		...
		*/
		if (!spectator) {
			addWidget(new GuiButtonIO(left + xSize + 2, top + ySize - 16, false, this, button -> {
				if (hasShiftDown()) {
	...
	public void addWidgets(List<Button> list) {
	...
	public void onSelectedChanged() {
		removeButtons(configWidget.configButtons);
		...
		if (selectedX != -1 && selectedY != -1) {
			...
			if (piece != null) {
				...
				if (piece.hasConfig()) {
					...
					configWidget.configButtons.forEach(this::addWidget);
	...
	public List<AbstractWidget> getButtons() {
		...
		this.children().forEach((child) -> children.add((AbstractWidget) child));
		return children;
	}
